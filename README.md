...
- **Custom floor & wall textures** — upload any tileable image (PNG, JPG, WebP, AVIF, or GIF) to override the procedural surface for the active preset. Floor tiles 4× and walls tile 2× across the scene; uploading a wall texture also forces walls to render on `studio` / `outdoor` so the upload can't be silently invisible. Bytes live in IndexedDB so a single texture can be many MB without bloating localStorage; metadata persists across reloads. **Reset scene** wipes both slots back to procedural defaults.
