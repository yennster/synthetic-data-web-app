...
- **Shift+drag to move** — hold Shift and click+drag any spawned object or physics-enabled imported asset to move it anywhere in 3D. The drag plane faces the camera, so pointer-right moves the object along the camera's right and pointer-up along the camera's up. Orbit to a top-down view to reposition on the floor, side view to lift, etc.
  - **Press Alt mid-drag** to switch into **depth mode**: vertical cursor motion now brings the object closer (cursor up) or farther (cursor down) along the camera's gaze direction. Release Alt to switch back to in-plane drag — the gesture re-anchors so there's no snap. Works great on trackpads since you don't have to fight with wheel events.
  - **Mouse wheel during drag** does the same closer/farther motion as Alt — useful when one hand is on the mouse and one isn't free for the keyboard.
