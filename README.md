...
- **Shift+drag to move** — hold Shift and click+drag any spawned object or physics-enabled imported asset to slide it across the belt or floor (horizontal XZ). Hold **Shift+Alt** while dragging to lift it vertically — drop the modifier mid-air and physics resumes, so you can let go from any height and watch the object fall onto the belt. Orbit/zoom keeps working without the modifier. Velocity is zeroed at the start of each drag so released objects fall cleanly under gravity instead of drifting.
