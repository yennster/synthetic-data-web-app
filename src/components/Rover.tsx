...
function RoverController() {
  ...
  useEffect(() => {
    ...
    // user drags an obstacle while the rover is mid-run.
    const obstacles: ObstacleDisc[] = useStore
      .getState()
      .robotObstacles.map((o) => ({ x: o.x, z: o.z, r: o.r }));
...
function RoverImuSampler({
  ...
  useFrame((_, dt) => {
    ...
    const obstacles = useStore.getState().robotObstacles;
