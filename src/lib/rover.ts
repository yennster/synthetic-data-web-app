...
/**
 * Procedurally place `count` obstacles inside a disc of radius `radius`
 * around the origin, with each obstacle at least `minSpacing` away from
 * every other one. Used by the "Randomize obstacles" button.
 *
 * Returns disc descriptors only — the renderer maps them onto pillars /
 * crates / cones with deterministic shape choice keyed off the index
 * so the same scene reads as a stable layout but the spatial pattern
 * is fresh each time.
 */
export function generateObstacles(
  ...
  const MAX_TRIES_PER_SLOT = 30;
  for (let i = 0; i < count; i++) {
    for (let attempt = 0; attempt < MAX_TRIES_PER_SLOT; attempt++) {
      const a = rng() * Math.PI * 2;
      // Bias to mid-radius so obstacles aren't all hugging the rim or origin.
      const r = radius * (0.4 + rng() * 0.55);
      const x = Math.cos(a) * r;
      const z = Math.sin(a) * r;
      const candidate: ObstacleDisc = {
        x,
        z,
        r: 0.18 + rng() * 0.12,
      };
      let ok = true;
      // Keep clear of the rover spawn zone.
      if (Math.sqrt(x * x + z * z) < 1.0) ok = false;
      if (ok) {
        for (const existing of out) {
          ...
          if (
            ...
            candidate.r + existing.r + minSpacing
          ) {
            ok = false;
            break;
          }
