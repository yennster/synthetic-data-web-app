...
5. Position the **Virtual camera** in the Virtual Camera card. The orange frustum gizmo updates live in the scene; Shift+drag the camera body to move it directly, and drag the preview's bottom-right corner to enlarge the captured framing.
...
7. Click **📸 Capture frame** for one image, or set a batch count + randomization toggles and click **⚡ Batch (N)** — both single shots and batches download as `.zip` files. For non-random camera trajectories, Shift+drag the pink orbit-center marker to move the point the path looks at and orbits around. Single-frame zips contain the PNG + a matching `bounding_boxes.labels`; batch zips contain every PNG plus one shared sidecar.
